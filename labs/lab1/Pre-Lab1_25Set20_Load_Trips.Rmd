...
requiredPackages <- c("missMDA", "mvoutlier", "effects","FactoMineR","car", "factoextra","RColorBrewer","ggplot2","dplyr","ggmap","ggthemes","knitr")
...
df<-read.table(paste0(filepath,"/course_data/NYCABS/green_tripdata_2016-01.csv"),header=T, sep=",")
dim(df)
names(df)
...
summary(df$RateCodeID)
      # consell -> ajuntar grups amb poques observacions
      # > summary(df$RateCodeID)
      #   1       2     3     4   5     6 
      #   4873    14    2     1   109   1 
      # pas 1: crear nova variable dins dataframe (inicialitzo a 1)
      df$f.Rate<-1  #rep(1,nrow(df))
      # pas 2: crear llista amb observacions sense grup 1
      ll<-which(df$RateCodeID!="1"); length(ll)
      # pas 3: per la nova variable, els hi poso codi 2
      df$f.Rate[ll]<-2
      # pas 4: poso els de 1 i de 2
      df$f.Rate<-factor(df$f.Rate, levels=1:2, labels=c("f.Rate-1", "f.Rate-Other"))
      
      summary(df$f.Rate); summary(df$RateCodeID)
...
names(df) # [6] "Pickup_longitude"
jerrs[6]<-length(sel)
sel                 #### sel contains the rownames of the individuals with "0"  as  value for longitude
...
llout<-which((df$Total_amount<0)|(df$Total_amount>100)) # outliers per sobre de 100
...
names(df) # depen de com tinguis les vers
vars_con<-names(df)[c(10:16,18,22:28)]  # variables numèriques
vars_dis<-names(df)[c(1,4,5,20:21,28)]  # variables factors
vars_res<-names(df)[c(19)]              # variables tablet
